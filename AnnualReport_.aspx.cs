...
namespace WebApp
{
    public partial class AnnualReport : System.Web.UI.Page
    {
        ...
        {
            if(yearList==null||yearList.Length==0) yearList=new[]{2024,2025};
            var dt = BuildTestData(yearList);
        ...
        {
            int[] years = Request.Form.GetValues("year")?.Select(int.Parse).ToArray() ?? new[]{2024,2025};
        ...
        private static DataTable BuildTestData(int[] years)
        {
            ...
            dt.Columns.AddRange(new[]{
                ...
                new DataColumn("Inch",typeof(string)),
                new DataColumn("CustItem",typeof(string)),
                new DataColumn("OAItem",typeof(string)),
                new DataColumn("Substrate",typeof(string)),
            ...
            {
                var r = dt.NewRow();
                r["Customer"]  = $"Cust{(i%4)+1}";
                r["Inch"]      = $"{6+(i%4)}\"";
                r["CustItem"]  = $"CI{(i%6)+1}";
                r["OAItem"]    = $"OA{(i%5)+1}";
                r["Substrate"] = (i%2==0)?"SubA":"SubB";
                r["Price"]     = Math.Round(100+5*i + rnd.NextDouble()*10, 2);
                ...
                foreach(int y in years)
                {
                    ...
                    r[$"{y}_Total"] =
                        (double)r[$"{y}_Q1_Total"]+(double)r[$"{y}_Q2_Total"]+
                        (double)r[$"{y}_Q3_Total"]+(double)r[$"{y}_Q4_Total"];
                }
                dt.Rows.Add(r);
            }
            return dt;
